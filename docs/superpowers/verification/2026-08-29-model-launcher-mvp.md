...
| Server settings parse/copy logic; dynamic refresh preserves unsaved drafts while live auth/token state stays current | `crates/model-launcher-ui/src/lib.rs`; `crates/model-launcher-ui/ui/app.slint` | View-model `server_settings_validate_inputs_and_derive_live_security_copy` passed. Windows-only ignored test `dynamic_refresh_preserves_unsaved_settings_and_current_log_filter` compiled but ran 0 tests on macOS. | Automated PASS for parse/copy logic; real-window dynamic refresh NOT RUN |
