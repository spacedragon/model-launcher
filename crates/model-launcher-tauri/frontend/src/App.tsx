...
type Page = "models" | "api" | "benchmark" | "logs" | "settings" | "detail";
const nav: Array<[Page, string, string]> = [
  ["models", "模型库", "◆"], ["api", "API 服务", "▦"], ["benchmark", "基准测试", "◫"],
...
export default function App() {
  ...
  return <div className="app">
    ...
    <div className={`body ${page === "logs" || page === "settings" || page === "benchmark" ? "no-aside" : ""}`}>
      ...
      {page !== "logs" && page !== "settings" && page !== "benchmark" && <aside>
...
function DetailPage({ model, data, back, save }: { model: Model; data: Bootstrap; back(): void; save(key: string, settings: LaunchSettings): void }) {
  ...
  return <section className="view"><div className="detail-head"><div><button className="crumb" onClick={back}>← 模型库</button><h1>{model.name} <span className={`tag ${model.running ? "green" : ""}`}>{model.running ? "运行中" : "模型级参数"}</span></h1></div><div className="grow" /><button className="btn" onClick={() => setSettings(data.engineSettings.defaults)}>恢复全局默认</button><button className="btn primary" onClick={() => save(key, settings)}>保存并{model.running ? "重启" : "加载"}</button></div>
    <div className="params scroll"><div className="model-facts">
      ...
      <div className="field"><label>投机解码 <span className="tag">--spec-type</span></label><div className="input"><select disabled={!data.capabilities.speculative_type} value={settings.speculative_type ?? ""} onChange={e => setSettings(s => ({...s, speculative_type: (e.target.value || undefined) as LaunchSettings["speculative_type"]}))}><option value="">关闭</option><option value="draft-mtp">MTP</option><option value="draft-dflash">DFlash</option></select></div><small>{data.capabilities.speculative_type ? "仅为带有兼容预测头的模型启用。" : "当前 llama-server 不支持 --spec-type。"}</small></div>
